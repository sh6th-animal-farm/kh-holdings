...
public class OrderRedisService {
	...
	private void executeMatching(OrderRequestDTO myOrderDTO) {
		...
		while (true) {
			...
			// (1) 체결 정산 - 비동기 정산 및 이력 저장용 (Stream)
			// OrderRedisService -> TradeWorker
			// trade:stream:{tokenId} 현금 및 토큰 정산, 주문 내역 업데이트, 거래 내역 추가
			...
			// OrderRedisService -> MarketWorker
			// trade:topic:{tokenId}
			redissonClient.getTopic(redisKeyManager.getTradeTopicKey(myOrderDTO.getTokenId()))
					.publish(transactionDTO);

			// (3) 현재가 갱신 (예: "ticker:last_price:{tokenId}")
			// getBucket: Redis에 해당 토큰의 마지막 체결가를 저장
			// ticker:last_price:
			redissonClient.getBucket(redisKeyManager.getLastPriceKey(myOrderDTO.getTokenId()))
					.set(targetPrice);

			log.info("[OrderRedisService] 체결: Price {}, Volume {}, Amount {}", targetPrice.toPlainString(), executedVolume.toPlainString(), executedAmount.toPlainString());
			log.info("[OrderRedisService] Worker에게 나머지 작업 전달: TradeID {}", tradeId);
			...
			if (targetOrderDTO.getRemainingToken().compareTo(BigDecimal.ZERO) <= 0) {
				...
				if (targetOrderDTO.getOrderType() == OrderType.LIMIT) {
					...
					redissonClient.getTopic(redisKeyManager.getOrderTopicKey(targetOrderDTO.getTokenId())).publish(event);
				}
			}
		}
	...
	private void finalizeOrderProcess(OrderRequestDTO myOrderDTO, OrderSide mySide) {
		...
		if (isCompleted) {
			...
			if (myOrderDTO.getOrderType() == OrderType.MARKET) {
				...
				redissonClient.getTopic(redisKeyManager.getTradeTopicKey(myOrderDTO.getTokenId())).publish(event);
			}
		}
	}
	...
	public void removeOrder(Long tokenId, OrderSide side, Long orderId) {
		...
		redissonClient.getTopic(redisKeyManager.getTradeTopicKey(tokenId)).publish(event);
	}
