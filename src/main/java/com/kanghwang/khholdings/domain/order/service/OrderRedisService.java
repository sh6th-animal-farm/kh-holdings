...
public class OrderRedisService {
	...
	private void executeMatching(OrderRequestDTO myOrderDTO) {
		...
		while (true) {
			...
			redissonClient.getStream(redisKeyManager.getPrefix() + "trade:stream:", new JsonJacksonCodec(objectMapper))
					.add(StreamAddArgs.entry("data", transactionDTO));

			// [MarketWorker - 체결]
			// (2) 실시간 프론트엔드 전파용 (Pub/Sub)
			// 체결 내역을 MarketWorker가 받아서 웹소켓으로 전송
			redissonClient.getTopic(redisKeyManager.getPrefix() + "trade:topic:" + myOrderDTO.getTokenId(), new JsonJacksonCodec(objectMapper))
			...
			if (targetOrderDTO.getRemainingToken().compareTo(BigDecimal.ZERO) <= 0) {
				...
				if (targetOrderDTO.getOrderType() == OrderType.LIMIT) {
					...
					redissonClient.getTopic(redisKeyManager.getPrefix() + "order:topic:" + targetOrderDTO.getTokenId(), new JsonJacksonCodec(objectMapper)).publish(event);
				}
			}
		}
	...
	private void finalizeOrderProcess(OrderRequestDTO myOrderDTO, OrderSide mySide) {
		...
		if (isCompleted) {
			...
			if (myOrderDTO.getOrderType() == OrderType.MARKET) {
				...
				redissonClient.getTopic(redisKeyManager.getPrefix() + "order:topic:" + myOrderDTO.getTokenId(), new JsonJacksonCodec(objectMapper)).publish(event);
			}
		}
	}
	...
	public void removeOrder(Long tokenId, OrderSide side, Long orderId) {
		...
		redissonClient.getTopic(redisKeyManager.getPrefix() + "order:topic:" + tokenId, new JsonJacksonCodec(objectMapper)).publish(event);
	}
